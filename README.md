...
### Automatic logger configuration file switch

Setup build can detect the current application logger, and manage the good xml file template.

Detection is based on project `pom.xml` (precisely `effective-pom`) dependencies, as:
...
```bash
runuser -u <SERVICE_NAME> -- java -Dlogging.config=/etc/<SERVICE_NAME>/log4j2.xml|logback.xml -jar /usr/lib/<SERVICE_NAME>/Spring Boot.jar --spring.config.location=/etc/<SERVICE_NAME>/application.yml
```
